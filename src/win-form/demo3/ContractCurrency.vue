      ...
      <form class="child-main-table">
        <ul class="child-table-lists">
          <li style="display: none;" class="table-list">
            <search-box placeholder="搜索币种"></search-box>
          </li>
          <li v-if="tableLists.length" @click="selectItemView(item)" v-for="item in tableLists" :key="item.Id" class="table-list">
            <form-list ref="formListUnit"
              :item="item"
              :showCheckBox="showCheckBox"
              :title="item.Currency"
              :left="item.ContractAmount + ''"
              :center="_formatDate(item.UpdDate)"
              :right="item.ContractAmountRMB + ''"
              :tagShow="tagShow"
    ...
    <div class="chile-table-lists-form" :style="{'transform': 'translate3d(' + this.offset + '%, 0, 0)'}">
      <form class="input-textarea-group">
        <div class="input-row">
          ...
          <input v-model="currentItem.Currency" type="text" class="input" placeholder="输入币种">
        </div>
        <div class="input-row">
          <label class="label-text">汇率</label>
          <input v-model="currentItem.ExchangeRate" type="text" class="input" placeholder="输入汇率">
        </div>
        <div class="input-row">
          <label class="label-text">币种金额</label>
          <input v-model="currentItem.ContractAmount" type="text" class="input" placeholder="输入币种金额">
        </div>
        <div class="input-row">
          <label class="label-text">人民币金额</label>
          <input v-model="currentItem.ContractAmountRMB" type="text" class="input" placeholder="输入人民币金额">
        </div>
      </form>
