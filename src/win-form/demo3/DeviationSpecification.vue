<template>
  <div ref="DeviationSpecification" class="chile-table-unit">
        ...
        <ul class="child-table-lists">
          ...
          <li v-if="tableLists.length"
            ...
            <form-list ref="formListUnit"
              ...
              :title="item.DeviationTerm"
              :left="item.CurrentExplain || '暂无'"
              :center="_formatDate(item.UpdDate)"
              :right="item.OriginalExplain || '暂无'"
              :tagShow="tagShow"
    ...
    <div class="chile-table-lists-form" :style="{'transform': 'translate3d(' + this.offset + '%, 0, 0)'}">
      <form class="input-textarea-group">
        <div class="input-row">
          <label class="label-text">偏差项</label>
          <input v-model="currentItem.DeviationTerm" type="text" class="input" placeholder="输入偏差项">
        </div>
        <div class="input-row">
          <label class="label-text">当前情况说明</label>
          <input v-model="currentItem.CurrentExplain" type="text" class="input" placeholder="输入当前情况说明">
        </div>
        <div class="input-row">
          <label class="label-text">原情况说明</label>
          <input v-model="currentItem.OriginalExplain" type="text" class="input" placeholder="输入原情况说明">
        </div>
      </form>
